    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                '''
                # EIOU, SIOU, ALPHA-IOU, FocalEIOU, Wise-IOU
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, EIoU=True, Focal=True, gamma=0.5)  # iou(prediction, target)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                '''
                # EIOU, SIOU, ALPHA-IOU, FocalEIOU, Wise-IOU
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, EIoU=True, Focal=True, gamma=0.5)  # iou(prediction, target)
                if type(iou) is tuple:
                    if len(iou) == 2:
                        lbox += (iou[1].detach() * (1 - iou[0])).mean()
                        iou = iou[0]
